...
class uart_rand_seq extends uvm_sequence #(uart_seq_item);
    ...
    task body();
        uart_seq_item item;
        `uvm_info("SEQ", $sformatf("시퀀스를 시작합니다. num_trans = %0d", num_trans), UVM_LOW)
...
class uart_scoreboard extends uvm_scoreboard;
        ...
        if (item.tx_data !== item.rx_data) begin
           ...
           pass_cnt++; 
            `uvm_info(get_type_name(), $sformatf(
                       "Match! tx_data=0x%02h, rx_data=0x%02h",  item.tx_data, item.rx_data),
                        UVM_MEDIUM)
        end
    endfunction
    ...
    function void report_phase(uvm_phase phase);
        ...
        `uvm_info(get_type_name(), $sformatf(" Total transactions: %0d", pass_cnt + fail_cnt),
                  UVM_LOW)
        ...
        if (fail_cnt > 0) begin
            ...
            `uvm_info(get_type_name(), $sformatf("TEST PASSED: %0d all matches detected!", pass_cnt), 
                      UVM_LOW)
        end
        `uvm_info(get_type_name(), "\n\n", UVM_LOW)
    endfunction
...
class uart_monitor extends uvm_monitor;
    ...
    task run_phase(uvm_phase phase);
        ...
        forever begin
            wait(u_if.reset === 1'b0);
            item = uart_seq_item::type_id::create("item");

            while (u_if.mon_cb.tx_start !== 1'b1) @(u_if.mon_cb);

            item.tx_data = u_if.mon_cb.tx_data;
            `uvm_info(get_type_name(), $sformatf("tx_data 캡처: tx_data=0x%02h", item.tx_data), UVM_HIGH)

            while (u_if.mon_cb.rx_valid !== 1'b1) @(u_if.mon_cb);
            item.rx_data = u_if.mon_cb.rx_data;
            `uvm_info(get_type_name(), $sformatf("rx_data 캡처: rx_data=0x%02h", item.rx_data), UVM_HIGH)
            ap.write(item);
            @(u_if.mon_cb);
        end
    endtask
...
class uart_driver extends uvm_driver #(uart_seq_item);
    ...
    task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            // =================================================================
            // [핵심 수정] fork ~ join을 사용하여 TX 상태와 RX 상태를 '동시에' 감시합니다.
            // 둘 중 어느 신호가 먼저 끝나더라도 펄스를 놓치지 않습니다!
            // =================================================================
            fork
                begin // [Thread 1]: TX가 끝날 때까지 감시
                    while(!u_if.drv_cb.tx_busy) @(u_if.drv_cb); 
                    while(u_if.drv_cb.tx_busy) @(u_if.drv_cb);  
                    `uvm_info(get_type_name(), $sformatf("전송 완료: tx_data = 0x%02h", item.tx_data), UVM_HIGH)
                end
                
                begin // [Thread 2]: RX가 들어올 때까지 감시
                    while(u_if.drv_cb.rx_valid !== 1'b1) @(u_if.drv_cb); // 3상태 비교로 안전하게!
                    item.rx_data = u_if.drv_cb.rx_data;
                    `uvm_info(get_type_name(), $sformatf("Driver 수신완료: rx_data = 0x%02h", item.rx_data), UVM_HIGH)
                end
            join
            // fork 안의 두 Thread가 모두 끝나야만 이쪽으로 넘어옵니다.
